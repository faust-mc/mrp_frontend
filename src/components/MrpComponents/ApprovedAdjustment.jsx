...
function ApprovedAdjustment({ setAdjustments, isEditable, numberOfItems,
                            deliveryMultiplier, approvedDate }) {

  const { idofinventory } = useParams();
  const [forecast, setForecast] = useState([]);
  const [loading, setLoading] = useState(true);
  const [adjustmentValues, setAdjustmentValues] = useState({});
  const [isFirstAdjustmentEditable, setIsFirstAdjustmentEditable] = useState(true);
  const [isSecondAdjustmentEditable, setIsSecondAdjustmentEditable] = useState(true);
  const [isThirdAdjustmentEditable, setIsThirdAdjustmentEditable] = useState(true);
            ...
            <>
             <td>{Math.ceil(weekly_forecast * deliveryMultiplier[1])}</td>
              <td>
                <input
                  type="number"
                  value={numberOfItems >= 2 ? adjustedValues.second_adjustment || 0 : ""}
            ...
            <>
             <td>{Math.ceil(weekly_forecast * deliveryMultiplier[2])}</td>
              <td>
                <input
                  type="number"
                  value={numberOfItems >= 3 ? adjustedValues.third_adjustment || 0 : 0}
...
export default ApprovedAdjustment;
